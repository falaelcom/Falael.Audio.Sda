...
def run(profiles_root: str, profile_names: str, paths: dict):
    ...
    for src_track_path in sda.list_dir(source_dir = paths['src'], globs = ("*.wav", "*.mp3", "*.flac")):
        ...
        # fulltrack images session
        session = sda.session(src_track_path)

        session.proc(processors_metrics.base_freq_response_fulltrack)
        session.proc(processors_metrics.base_stereo_width_fulltrack)
        session.proc(processors_visualization.image_fulltrack)

        session.export_files_begin(f"{out_dir}/images-fulltrack/")
        session.export_files(processors_visualization.image_fulltrack, select=lambda data: (data["result"]["dir_name"], [data["result"]["file_name"]]))
        session.export_files_end()

        # transform processing - step 1 - stereo phase
        tmp_dir = f"{paths['tmp']}/1/{track}/"
