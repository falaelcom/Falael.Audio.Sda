def normalize_value(raw_value, norm_config):
    ...
    return normalized, norm_config["polarity"]

def get_data_view(previous: dict, dimensions: list, metrics: dict, chunk_duration_sec: dict) -> dict:
    """
    ...
    Returns:
        Tree structure: z_key -> y_key -> x_key -> {value: float, polarity: str}
    ...
    for metric_idx, metric_full_name in enumerate(metrics.keys()):
        ...
        if len(bucket_data) == 0:
            raise ValueError(f"Bucket '{bucket_name}' has not been populated.")
        ...
        for band_idx, (band_range, chunk_list, original_label) in enumerate(sorted_bands):
            for chunk_data in chunk_list:
                ...
                start_time = chunk_number * chunk_duration_sec
                end_time = (chunk_number + 1) * chunk_duration_sec
                ...
                data_obj = {
                    ...
                    "value": raw_value,
                    "metric_config": metrics[metric_full_name]
                }
    ...
    for obj in data_objects:
        ...
        if obj['value'] is None:
            ...
            metric_polarity = obj["metric_config"]["polarity"]
        else:
            normalized_value, metric_polarity = normalize_value(obj["value"], obj["metric_config"])
